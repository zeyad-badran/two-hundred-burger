          ...
          <div className="bg-char-soft border border-char-line rounded-lg p-6 mb-8 text-left">
            ...
            <div className="flex flex-col gap-1 text-sm mb-2 text-cream-muted">
              <div className="flex justify-between">
                <span>نوع الطلب</span>
                <span className="text-left max-w-[60%] leading-relaxed">{order.delivery_address === 'Pickup' || !order.delivery_address ? 'استلام من الفرع' : order.delivery_address}</span>
              </div>
              {order.delivery_map_url && (
                <a href={order.delivery_map_url} target="_blank" rel="noopener noreferrer" className="text-sear text-left text-xs hover:underline">
                  عرض موقع الخريطة
                </a>
              )}
            </div>
            <div className="flex justify-between text-sm mb-2 text-cream-muted">
              ...
              <span className="font-ticket text-cream font-sans dir-ltr">{order.subtotal.toFixed(2)} JOD</span>
            </div>
            {order.delivery_fee !== undefined && order.delivery_fee > 0 && (
              <div className="flex justify-between text-sm mb-2 text-cream-muted">
                <span>رسوم التوصيل</span>
                <span className="font-ticket text-cream font-sans dir-ltr">{order.delivery_fee.toFixed(2)} JOD</span>
              </div>
            )}
            <div className="flex justify-between text-sm font-bold text-cream mt-2 pt-2 border-t border-char-line">
              <span>المجموع الكلي</span>
              <span className="font-ticket text-sear font-sans dir-ltr">{(order.subtotal + (order.delivery_fee || 0)).toFixed(2)} JOD</span>
            </div>
          </div>
