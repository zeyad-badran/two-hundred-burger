...
const DeliveryMapPicker = dynamic(() => import('@/components/checkout/DeliveryMapPicker'), { 
  ssr: false,
  loading: () => <div className="w-full h-[300px] bg-char-soft animate-pulse rounded-lg border border-char-line flex items-center justify-center text-cream-muted">Loading map...</div>
});
...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || /\d/.test(formData.name)) {
      newErrors.name = 'Please enter a valid name (cannot contain numbers).';
      hasError = true;
    }

    if (!/^(962|07)\d+$/.test(formData.phone)) {
      newErrors.phone = 'Please enter a valid phone number starting with 962 or 07 (e.g. 0791234567).';
      hasError = true;
    }
    ...
    if (formData.fulfillment === 'delivery') {
      ...
      if (deliveryLat === null && deliveryLng === null) {
        newErrors.map = 'Please pin your delivery location on the map.';
        hasError = true;
    ...
    try {
      ...
      const payload = {
        ...
        branchId: branchId,
        branchName: selectedBranch?.name_en || '',
        branchAddress: selectedBranch?.address_en || '',
                      ...
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                            branchId === branch.id ? 'border-sear' : 'border-char-line'
                          }`}
                        >
                          {branchId === branch.id && (
                            <div className="w-2.5 h-2.5 rounded-full bg-sear" />
                          )}
                        </div>
                        <div>
                          <span className="font-ticket uppercase tracking-widest">{branch.name_en}</span>
                          <p className="text-sm mt-1">{branch.address_en}</p>
                        </div>
                      </div>
                      <input
                        type="radio"
            ...
            <div className="space-y-2">
                    ...
                    {deliveryDistance !== null && selectedBranchConfig && (
                      <div className={`mt-4 p-4 rounded-lg border ${isDeliveryInRange ? 'border-sear/30 bg-sear/5' : 'border-ember/30 bg-ember/5'}`}>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="font-ticket text-cream">{deliveryDistance.toFixed(1)} km</span>
                        </div>
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-sm text-cream-muted">{dict.ui.checkout.deliveryRadius}:</span>
                          <span className="font-ticket text-cream">{selectedBranchConfig.delivery_radius_km} km</span>
                        </div>
                        {isDeliveryInRange ? (
                          <>
                            <div className="flex justify-between items-center mb-3">
                              <span className="text-sm text-cream-muted">{dict.ui.checkout.deliveryFee}:</span>
                              <span className="font-ticket text-sear">{deliveryFee.toFixed(2)} {dict.ui.cart.currency}</span>
                            </div>
              ...
              <div className="border-t border-char-line pt-6">
                ...
                <div className="flex justify-between items-center mb-3">
                  ...
                  <span className="font-ticket text-cream">{subtotal.toFixed(2)} {dict.ui.cart.currency}</span>
                </div>
                {formData.fulfillment === 'delivery' && (
                  <div className="flex justify-between items-center mb-3 text-cream-muted">
                    ...
                    <span className="font-ticket">{deliveryFee.toFixed(2)} {dict.ui.cart.currency}</span>
                  </div>
                )}
                <div className="flex justify-between items-center mb-6 pt-3 border-t border-char-line">
                  ...
                  <span className="font-ticket text-xl text-sear">{finalTotal.toFixed(2)} {dict.ui.cart.currency}</span>
                </div>
